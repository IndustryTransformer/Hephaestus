...
"""
TransformerEncoderBlock(nhead::Int, dm::Int, dhid::Int; pdrop::Float64 = 0.1, act = relu) =
...
function _show_transformer_encoder(io::IO, t::TransformerEncoderBlock, indent = 0)
    ...
    for layer in [
        t.dropout,
        t.layer_norm_attention,
        t.dense1,
        t.dense2,
        t.dropout,
        t.layer_norm_feedforward,
    ]
        Lux._layer_show(io, layer, inner_indent)
    end
    print(io, " "^indent, ")")
    if indent == 0
        Lux._big_finale(io, t)
    else
        println(io, ",")
    end
end

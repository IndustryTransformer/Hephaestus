...
function _show_multiheadattention(io::IO, mha::MultiheadAttention, indent = 0)
    ...
    print(io, " "^indent, mha, "(\n")
    Lux._layer_show(io, mha.denseQ, inner_indent, "denseQ")
    Lux._layer_show(io, mha.denseK, inner_indent, "denseK")
    Lux._layer_show(io, mha.denseV, inner_indent, "denseV")
    Lux._layer_show(io, mha.denseO, inner_indent, "denseO")
    print(io, " "^indent, ")")
    if indent == 0
        Lux._big_finale(io, mha)
    else
        println(io, ",")
    end
end
...
function (mha::MultiheadAttention)(
    ...
    key::A2,
    value::A3,
) where {T,A1<:AbstractArray{T,3},A2<:AbstractArray{T,3},A3<:AbstractArray{T,3}}
...
function (mha::MultiheadAttention)(
    ...
    key::A2,
    value::A3,
) where {T,A1<:AbstractMatrix{T},A2<:AbstractMatrix{T},A3<:AbstractMatrix{T}}
...
function scaled_dot_attention(
    ...
    key::A2,
    value::A3,
) where {T,A1<:AbstractArray{T,4},A2<:AbstractArray{T,4},A3<:AbstractArray{T,4}}

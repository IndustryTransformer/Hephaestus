...
class SparseAttention4D(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # Apply sparse mask
        scores = scores.masked_fill(~sparse_mask.unsqueeze(0).unsqueeze(0).unsqueeze(0), float('-inf'))
...
class FlashAttention4D(nn.Module):
    ...
    def __init__(
        ...
        self.dropout = dropout  # Note: dropout is handled by scaled_dot_product_attention
